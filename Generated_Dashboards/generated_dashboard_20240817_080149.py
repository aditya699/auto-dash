...
def create_app(df):
    ...
    @callback(
        ...
        [Input('dropdown-department', 'value'),
         ...
         Input('department-salary', 'clickData'),
         Input('gender-distribution', 'clickData'),
         Input('age-distribution', 'selectedData'),
         Input('reset-button', 'n_clicks')]
    )
    def update_dashboard(departments, job_titles, start_date, end_date, salary_selection, department_click, gender_click, age_selection, reset_clicks):
        ...
        if ctx.triggered:
            ...
            elif input_id == 'department-salary' and department_click:
        ...
        salary_distribution = px.histogram(dff, x='salary', nbins=30, title="Salary Distribution",
                                           color_discrete_sequence=[colors['primary']], labels={'salary':'Salary'})
        salary_distribution = update_chart_layout(salary_distribution)
        
        department_salary = px.bar(dff.groupby('department')['salary'].mean().reset_index(), 
                                   x='department', y='salary', title="Average Salary by Department",
                                   color_discrete_sequence=[colors['primary']], labels={'department':'Department', 'salary':'Average Salary'})
        ...
        dff['age'] = datetime.now().year - dff['date_of_birth'].dt.year
        ...
        return kpi_indicators, salary_distribution, department_salary, gender_distribution, age_distribution
