...
def create_app(df):
    ...
    def update_dashboard(departments, start_date, end_date, salary_selection, department_click, gender_click, age_selection, reset_clicks):
        ...
        department_breakdown = px.pie(dff, names='department', values='employee_id', title="Department Breakdown",
                                      color_discrete_sequence=px.colors.qualitative.Set3)
        department_breakdown = update_chart_layout(department_breakdown)
        
        gender_distribution = px.pie(dff, names='gender', values='employee_id', title="Gender Distribution",
                                     color_discrete_sequence=[colors['primary'], colors['secondary']])
        gender_distribution = update_chart_layout(gender_distribution)
        
        age_distribution = px.histogram(dff, x=datetime.now().year - dff['date_of_birth'].dt.year, nbins=20, title="Age Distribution",
