...
def create_app(df):
    ...
    def update_dashboard(departments, start_date, end_date, salary_selection, department_click, gender_click, age_selection, reset_clicks):
        ...
        def create_kpi_card(title, value, format_func=lambda x: f'{x:,.0f}'):
            return html.Div([
                html.H3(title, style={'color': colors['text'], 'marginBottom': '10px', 'fontSize': '16px', 'fontWeight': '400'}),
                html.Div([
                    html.Span(format_func(value), style={'fontSize': '28px', 'fontWeight': 'bold', 'color': colors['primary']})
                ])
            ], style=kpi_style)
        
        kpi_indicators = html.Div([
            create_kpi_card('Total Employees', total_employees),
            create_kpi_card('Average Salary', avg_salary, lambda x: f'${x:,.0f}'),
            create_kpi_card('Gender Diversity', gender_diversity, lambda x: ', '.join([f'{k}: {v:.1%}' for k, v in x.items()])),
            create_kpi_card('Average Age', avg_age, lambda x: f'{x:.1f} years')
        ...
        gender_distribution = px.pie(dff, names='gender', title="Gender Distribution",
                                     color_discrete_sequence=[colors['primary'], colors['secondary']], labels={'gender': 'Gender'})
        ...
        return kpi_indicators, salary_distribution, department_breakdown, gender_distribution, age_distribution    
