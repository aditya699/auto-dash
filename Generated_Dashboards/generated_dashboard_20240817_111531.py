...
def create_app(df):
    ...
    def update_dashboard(departments, start_date, end_date, salary_selection, department_click, age_selection, gender_click, reset_clicks):
        ...
        kpi_indicators = html.Div([
            ...
            html.Div([
                html.H3('Gender Diversity', style={'color': colors['text'], 'marginBottom': '10px', 'fontSize': '16px', 'fontWeight': '400'}),
                html.Div([
                    html.Span(f"M: {gender_diversity.get('Male', 0):.0%}", style={'fontSize': '18px', 'color': colors['primary']}),
                    html.Span(f"F: {gender_diversity.get('Female', 0):.0%}", style={'fontSize': '18px', 'color': colors['secondary'], 'marginLeft': '10px'})
                ])
            ], style=kpi_style)
        ...
        return kpi_indicators, salary_distribution, department_distribution, age_distribution, gender_distribution
