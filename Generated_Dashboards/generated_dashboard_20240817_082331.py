...
def create_app(df):
    ...
    app.layout = dbc.Container([
        html.Div([
            ...
            dbc.Row([
                dbc.Col([dcc.Graph(id='sales-by-product')], width=6),
                dbc.Col([dcc.Graph(id='sales-by-gender')], width=6),
            ], className='mb-4'),
    ...
    @callback(
        ...
        [Input('dropdown-gender', 'value'),
         ...
         Input('sales-by-product', 'clickData'),
         Input('sales-by-gender', 'clickData'),
    ...
    def update_dashboard(genders, products, start_date, end_date, product_click, gender_click, time_selection, payment_click, reset_clicks):
        ...
        if ctx.triggered:
            ...
            if input_id == 'reset-button':
                dff = df[df.gender.isin(genders) & 
                         ...
                         (df.purchase_date >= start_date) & 
                         (df.purchase_date <= end_date)]
            elif input_id == 'sales-by-product' and product_click:
                product = product_click['points'][0]['x']
                dff = dff[dff['product_name'] == product]
            elif input_id == 'sales-by-gender' and gender_click:
        ...
        sales_by_product = px.bar(dff.groupby('product_name')['total_price'].sum().reset_index(), 
                                  x='product_name', y='total_price', title="Sales by Product",
                                  color_discrete_sequence=[colors['primary']], 
                                  labels={'product_name': 'Product Name', 'total_price': 'Total Sales'})
        sales_by_product = update_chart_layout(sales_by_product)
        
        sales_by_gender = px.pie(dff, names='gender', values='total_price', title="Sales by Gender",
                                 color_discrete_sequence=[colors['primary'], colors['secondary']], 
                                 labels={'gender': 'Gender', 'total_price': 'Total Sales'})
        sales_by_gender = update_chart_layout(sales_by_gender)
        
        sales_over_time = px.line(dff.groupby('purchase_date')['total_price'].sum().reset_index(), 
                                  x='purchase_date', y='total_price', title="Sales Over Time",
                                  color_discrete_sequence=[colors['primary']], 
                                  labels={'purchase_date': 'Date', 'total_price': 'Total Sales'})
        sales_over_time = update_chart_layout(sales_over_time)
        
        payment_method_distribution = px.pie(dff, names='payment_method', title="Payment Method Distribution",
        ...
        return kpi_indicators, sales_by_product, sales_by_gender, sales_over_time, payment_method_distribution
