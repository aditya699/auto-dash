...
def create_app(df):
    ...
    app.layout = dbc.Container([
        html.Div([
            ...
            dbc.Row([
                ...
                dbc.Col([dcc.Graph(id='department-headcount')], width=6),
            ], className='mb-4'),
    ...
    @callback(
        [Output('kpi-indicators', 'children'),
         ...
         Output('department-headcount', 'figure'),
        ...
        [Input('dropdown-department', 'value'),
         ...
         Input('department-headcount', 'clickData'),
    ...
    def update_dashboard(departments, start_date, end_date, salary_selection, department_click, gender_click, age_selection, reset_clicks):
        ...
        if ctx.triggered:
            ...
            if input_id == 'reset-button':
            ...
            elif input_id == 'department-headcount' and department_click:
                ...
                dff = dff[(dff['date_of_birth'].dt.year.max() - dff['date_of_birth'].dt.year >= age_range[0]) & 
                          (dff['date_of_birth'].dt.year.max() - dff['date_of_birth'].dt.year <= age_range[1])]

        total_employees = len(dff)
        avg_salary = dff['salary'].mean()
        gender_ratio = dff['gender'].value_counts(normalize=True).get('Female', 0) * 100
        avg_age = (pd.Timestamp.now() - dff['date_of_birth']).mean().days / 365.25
        ...
        past_avg_age = (pd.Timestamp.now() - past_dff['date_of_birth']).mean().days / 365.25
        ...
        def create_kpi_card(title, value, change, format_str='{:,.0f}'):
            return html.Div([
                ...
                html.Div([
                    ...
                    html.Div([
                        ...
                        html.Span(format_str.format(abs(change)), 
        ...
        kpi_indicators = html.Div([
            ...
            create_kpi_card('Avg Salary', avg_salary, salary_change, '${:,.0f}'),
            create_kpi_card('Female Ratio', gender_ratio, gender_ratio_change, '{:.1f}%'),
            create_kpi_card('Avg Age', avg_age, age_change, '{:.1f}')
        ...
        department_headcount = px.bar(dff.groupby('department').size().reset_index(name='count'), 
                                      x='department', y='count', title="Department Headcount",
                                      color_discrete_sequence=[colors['primary']], labels={'department':'Department', 'count':'Headcount'})
        department_headcount = update_chart_layout(department_headcount)
        ...
        dff['age'] = (pd.Timestamp.now() - dff['date_of_birth']).astype('<m8[Y]')
        ...
        return kpi_indicators, salary_distribution, department_headcount, gender_distribution, age_distribution    
